...
internal class IshtarStackFrame : IshtarObject<StackFrame, StackFrameFormat>
{
    #region Private Fields

    private List<IshtarScope> childScopes;
    private IReadOnlyCollection<IshtarScope> _scopes;

    private IshtarScope argsScope;
    private IshtarScope localsScope;

    #endregion

    #region Constructor

    internal IshtarStackFrame(IshtarDebugAdapter adapter, RuntimeIshtarModule module, string functionName, IEnumerable<SampleFunctionArgument> args, string fileName, int line, int column)
        : base(adapter)
    {
        this.childScopes = new List<IshtarScope>();

        this.localsScope = new IshtarScope(this.Adapter, "Locals", false);
        ...
        if (args != null && args.Any())
        {
            ...
            this.argsScope = new IshtarScope(this.Adapter, "Arguments", false);
    ...
    internal RuntimeIshtarModule Module { get; private set; }
    ...
    internal void AddScope(IshtarScope scope)
    {
    ...
    /// </summary>
    internal IReadOnlyCollection<IshtarScope> ModifiableScopes
    {
        get
        {
            List<IshtarScope> scopes = new List<IshtarScope>();
    ...
    /// </summary>
    internal IReadOnlyCollection<IshtarScope> MergedScopes
    {
    ...
    /// </summary>
    internal IReadOnlyCollection<IshtarScope> AllScopes
    {
        get
        {
            List<IshtarScope> scopes = new List<IshtarScope>();
    ...
    private IEnumerable<IshtarScope> GetScopes()
    {
        IshtarScope aggregateScope = null;
        ...
        if (this.argsScope != null && this.argsScope.ChildContainers.Any())
        {
            ...
            else
            {
                ...
                {
                    aggregateScope = new IshtarScope(this.Adapter, "Locals", false);
                }

                foreach (IshtarVariable variable in this.argsScope.ChildContainers)
                {
        ...
        if (this.Adapter.GlobalsScope != null && this.Adapter.GlobalsScope.ChildContainers.Any() && this.Adapter.ShowGlobals)
        {
            ...
            else
            {
                ...
                foreach (IshtarVariable variable in this.Adapter.GlobalsScope.ChildContainers)
                {
                    aggregateScope.AddVariable(variable);
                }
            }
        ...
        if (aggregateScope != null)
        {
            ...
            foreach (IshtarVariable variable in this.localsScope.ChildContainers)
            {
                aggregateScope.AddVariable(variable);
            }
        ...
        foreach (IshtarScope scope in this.childScopes)
        {
            yield return scope;
        }
    }
    ...
    public override void Invalidate()
    {
        ...
        {
            foreach (IshtarScope scope in this._scopes)
            {
                scope.Invalidate();
            }
    ...
    {
        IIshtarVariableContainer container = null;
        ...
        IshtarVariable variable = container.Variables.FirstOrDefault(v => String.Equals(v.Name, variableName, StringComparison.Ordinal));
        if (variable == null)
        {
            throw new ProtocolException("Evaluation failed!");
        }
