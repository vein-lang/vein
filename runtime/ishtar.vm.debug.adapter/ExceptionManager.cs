...
internal class ExceptionManager
{
    ...
    private class ExceptionCategorySettings
    {
        ...
        internal void SetExceptionBreakMode(string exception, ExceptionBreakMode breakMode) => this.exceptionSettings.Add(exception, breakMode);
        ...
        public override string ToString() => Invariant($"{this.CategoryId:D}: {this.CategoryBreakMode}");
    }

    private Dictionary<string, ExceptionCategorySettings> exceptionCategorySettings;

    internal ExceptionManager(IshtarDebugAdapter adapter)
    {
    ...
    internal bool HasPendingException => this.pendingException != null;

    internal int PendingExceptionThread { get; private set; }

    internal void Invalidate() => this.pendingException = null;
    ...
    {
        IshtarThread thread = this.adapter.ThreadManager.GetThread(args.ThreadId);
        if (thread == null)
        {
