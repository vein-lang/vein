...
internal class ThreadManager
{
    ...
    private List<IshtarThread> threads;

    internal ThreadManager(IshtarDebugAdapter adapter)
    {
        this.adapter = adapter;
        this.threads = new List<IshtarThread>();
    ...
    internal IshtarThread StartThread(int id, string name)
    {
        IshtarThread newThread = new IshtarThread(id, name);
    ...
    internal void EndThread(IshtarThread thread)
    {
    ...
    internal IshtarThread GetThread(int threadId)
    {
        return this.threads.FirstOrDefault(t => t.Id == threadId);
    }

    internal IshtarStackFrame GetStackFrame(int stackFrameId)
    {
        return this.threads.SelectMany(t => t.StackFrames).FirstOrDefault(f => f.Id == stackFrameId);
    }

    internal IIshtarVariableContainer GetVariableContainer(int variablesReference) =>
        this.threads
    ...
    private IIshtarVariableContainer FindVariableReference(IIshtarVariableContainer container, int variablesReference)
    {
        ...
        {
            foreach (IIshtarVariableContainer childContainer in container.ChildContainers)
            {
                IIshtarVariableContainer found = this.FindVariableReference(childContainer, variablesReference);
                if (found != null)
                    return found;
            }
        }
    ...
    {
        foreach (IshtarThread thread in this.threads)
        {
            thread.Invalidate();
        }
    }
    ...
    internal ThreadsResponse HandleThreadsRequest(ThreadsArguments args)
        => new ThreadsResponse(threads: threads.Select(t => t.GetProtocolThread()).ToList());

    internal ScopesResponse HandleScopesRequest(ScopesArguments args)
    {
        IshtarStackFrame frame = this.GetStackFrame(args.FrameId);
        if (frame == null)
    ...
    {
        IshtarThread thread = this.GetThread(args.ThreadId);
        if (thread == null)
    ...
    {
        IIshtarVariableContainer container = this.GetVariableContainer(args.VariablesReference);
        if (container == null)
    ...
    internal EvaluateResponse HandleEvaluateRequest(EvaluateArguments args)
    {
        ...
        IshtarStackFrame frame = this.GetStackFrame(args.FrameId.Value);
        if (frame == null)
        {
    ...
    private bool DoEndThread(EndThreadArgs args, StringBuilder output)
    {
        ...
        IshtarThread thread = this.threads.FirstOrDefault(t => t.Id == args.Id);
        if (thread == null)
        {
    ...
    private bool DoPushStackFrame(PushStackFrameArgs args, StringBuilder output)
    {
        ...
        RuntimeIshtarModule module = null;
        ...
        IshtarThread targetThread = this.GetThread(args.ThreadId);
        ...
        targetThread.PushStackFrame(new IshtarStackFrame(this.adapter, module, args.Name, stackArgs, this.adapter.Source.Path, 0, 0));
    ...
    private bool DoDefineScope(DefineScopeArgs args, StringBuilder output)
    {
        ...
        IshtarStackFrame frame = thread.GetTopStackFrame();
        if (frame == null)
        {
        ...
        IshtarScope scope = new IshtarScope(this.adapter, args.Name, args.Expensive);
