...
internal class RegisterMask
{
    ...
    public bool IsEmpty => _gp == 0 && _mm == 0 && _k == 0 && _xyz == 0;
    ...
    public int Get(RegisterClass rc) => rc switch
    {
        RegisterClass.Gp => _gp,
        RegisterClass.Mm => _mm,
        RegisterClass.K => _k,
        RegisterClass.Xyz => _xyz,
    ...
    public bool Has(RegisterClass rc, int mask) =>
        rc switch
        {
            RegisterClass.Gp => _gp.IsSet(mask),
            RegisterClass.Mm => _mm.IsSet(mask),
            RegisterClass.K => _k.IsSet(mask),
            RegisterClass.Xyz => _xyz.IsSet(mask),
            _ => throw new InvalidEnumArgumentException(),
        };

    public override string ToString()
        => string.Format("Gp={0}, Mm={1}, K={2}, Xyz={3}", _gp, _mm, _k, _xyz);
}