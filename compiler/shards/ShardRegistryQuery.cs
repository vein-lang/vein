...
public class ShardRegistryQuery
{
        ...
        CancellationToken token = default)
    {
        ...
        using (var file = package.OpenWrite())
        using (var remote = await result.GetStreamAsync())
            await remote.CopyToAsync(file, token);

        var shard = await Shard.OpenAsync(package);
    ...
    public async ValueTask<RegistryPackage> DownloadShardAsync(string name, string version,
        CancellationToken token = default)
    {
        var manifest = await FindByName(name, version);
