namespace mana.lsp
{
    ...
    public class ManaLanguageServer : IDisposable
    {
        ...
        public ManaLanguageServer(Stream? sender, Stream? reader)
        {
            ...
            {
                SynchronizationContext = new ManaSynchronizationContext(),
            };
            this.rpc.StartListening();
            this.disconnectEvent = new ManualResetEvent(false);
            // let's make the server exit if the stream is disconnected
            this.rpc.Disconnected += (s, e) => this.disconnectEvent.Set();
            this.ReadyForExit = false;

            this.internalErrorTimer = new System.Timers.Timer(60000);
            this.internalErrorTimer.Elapsed += (_, _) => this.showInteralErrorMessage = true;
        ...
        public void WaitForShutdown() => this.disconnectEvent.WaitOne();
        ...
        /// <summary>
        /// to be called when the server encounters an internal error (i.e. a ManaCompilerError is raised)
        /// </summary>
        ...
        public object Initialize(JToken arg)
        {
            ...
            capabilities.ExecuteCommandProvider.Commands = new[] { CommandIds.ApplyEdit };
        ...
        public object OnTextDocumentDefinition(JToken arg)
        {
            ...
            var defaultLocation = new Location
            {
                ...
                Range = new Microsoft.VisualStudio.LanguageServer.Protocol.Range { Start = param.Position, End = param.Position },
            };
            try
            {
                return ManaCompilerError.RaiseOnFailure(
        ...
        public object? OnTextDocumentCompletionResolve(JToken arg)
        {
            ...
            try
            {
                ...
                return (data != null) ? ManaCompilerError.RaiseOnFailure(
                    () => Task.CompletedTask,
        ...
        public object OnCodeAction(JToken arg)
        {
            ...
            try
            {
                ...
                //ManaCompilerError.RaiseOnFailure(
        ...
        public void OnDidChangeWatchedFiles(JToken arg)
        {
            ...
            FileEvent[] PreprocessEvent(FileEvent fileEvent)
            {
                ...
                if (fileName.EndsWith(".mana") && fileEvent.FileChangeType != FileChangeType.Changed)
                {
                    bool FileIsWithinProjectDir(Uri projFile)
                    {
                        ...
                        ManaCompilerError.Verify(projDir != null, "could not determine project directory");
