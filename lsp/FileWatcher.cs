namespace mana.lsp
{
    ...
    internal class FileWatcher
    {
        ...
        private void OnBufferOverflow(object sender, ErrorEventArgs e)
        {
            ...
            try
            {
                ManaCompilerError.Raise($"buffer overflow in file system watcher: \n{e.GetException()}");
            }
            catch (Exception ex)
            {
                this.onException(ex);
        ...
        private static bool GlobDirectoryStructure(Dictionary<Uri, ImmutableHashSet<string>> directories, string path, IEnumerable<string> globPatterns)
        {
            ...
            var patterns = globPatterns as string[] ?? globPatterns.ToArray();
            success = patterns.TryEnumerate(
                          pattern => Directory.EnumerateFiles(root.LocalPath, pattern, SearchOption.TopDirectoryOnly), out var files)
                      && success;

            directories[root] = subfolders
                .Concat(files.SelectMany(items => items))
                .SelectNotNull(Path.GetFileName)
                .ToImmutableHashSet();

            return Enumerable.Aggregate(subfolders, success,
                (current, subfolder) => GlobDirectoryStructure(directories, subfolder, patterns) && current);
        }
